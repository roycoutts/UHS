<#
.SYNOPSIS
    Generates a table of colors from the System.Windows.Media.Brushes class and exports it to an HTML file.

.DESCRIPTION
    The script consists of two main functions: Get-ColorTable and Export-ColorTableToHTML. 
    - Get-ColorTable retrieves all colors from the System.Windows.Media.Brushes class and creates a custom object for each color, including details like color name, ARGB values, hex representation, decimal values, and HTML color code.
    - Export-ColorTableToHTML takes the generated color table and exports it to an HTML file, providing a visual representation of each color.

.PARAMETER ColorTable
    An array of custom objects representing colors and their properties. This parameter is mandatory for the Export-ColorTableToHTML function.

.PARAMETER OutputFilePath
    The file path where the HTML file will be saved. If not specified, the default location is the user's temp folder with the filename 'ColorTable.html'.
...
.EXAMPLE
    ...
    $outputPath = Export-ColorTableToHTML -ColorTable $ColorTable -OutputFilePath "C:\Users\UserName\Desktop\ColorTable.html"
